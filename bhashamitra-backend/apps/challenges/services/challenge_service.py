...
class ChallengeService:
    ...
    def get_available_categories(cls, language: str) -> List[Dict[str, Any]]:
        ...
        # FIXED: Using Count('id') instead of Sum('word_count')
        ...
        grammar_count = GrammarRule.objects.filter(
            topic__language=lang_upper, topic__is_active=True
        ).exclude(examples__isnull=True).count()

        if grammar_count > 0:
            available.append({"value": ChallengeCategory.MIMIC, "label": "Peppi Mimic", "item_count": grammar_count})
    ...
    @staticmethod
    def strip_answers(questions: List[Dict[str, Any]]) -> List[Dict[
def get_available_categories(cls, language: str) -> List[Dict[str, Any]]:
        available = []
        lang_upper = language.upper()
