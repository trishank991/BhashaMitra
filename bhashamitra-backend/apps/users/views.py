...
# TEMPORARY: Seed test users on production (remove after use)
class SeedTestUsersView(APIView):
    """Temporary endpoint to seed test users on production database."""
    permission_classes = [AllowAny]
    throttle_scope = 'auth'
    ...
    def post(self, request):
        ...
        secret = request.headers.get('X-Seed-Secret') or os.environ.get('SEED_USERS_SECRET', '')
        expected_secret = os.environ.get('SEED_USERS_SECRET', 'dev-seed-secret-change-in-prod')
        
        if not secret or secret != expected_secret:
            logger.warning(f"Unauthorized seed attempt from {request.META.get('REMOTE_ADDR')}")
            return Response(
                {'detail': 'Unauthorized'},
                status=status.HTTP_401_UNAUTHORIZED
            )
        ...
        for user_data in test_users:
            ...
            user.tts_provider = user_data['tts_provider']
            user.story_limit = user_data['story_limit']
            
            # Set subscription expiration for paid tiers
            if user_data['tier'] != 'FREE':
                user.subscription_expires_at = timezone.now() + timedelta(days=user_data['expires_in_days'])
            else:
        ...
        logger.info(f"Seed complete: {created} created, {updated} updated")
        
        return Response({
            'success': True,
            'message': f'Successfully seeded {len(test_users)} test users',
            'users': {
